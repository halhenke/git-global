...
pub fn get_results(raw_arg: bool) -> Result<GitGlobalResult> {
    ...
    if raw_arg {
        let mut f =
            File::open(config.cache_file).expect("No cache file found.");
        // let mut reader: Vec<u8> = Vec::new();
        // let reader = &mut Vec::new();
        let mut reader = String::new();
        f.read_to_string(&mut reader).expect("Couldnt read ");
    ...
    // result.add_message(config.cache_file.to_str().unwrap().to_string());
        ...
        result.add_message(format!(
            "For contents of cache file pass \"{}\" flag i.e.",
            "raw".blue()
        ));
        result.add_message(format!(
            "{}",
            ">>> git-global info raw".green().underline()
        ));
    }
    Ok(result)
}
